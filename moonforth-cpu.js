...
CPU.prototype = {
    ...
    step: function() {
        if(!this.paused) {
            ...
            switch (insn.opcode) {
                ...
                // HWI
                case 0x240:
                    var hwid = (aVal << 16) | this.get(insn.a + 1);
                    if(this._devices[hwid] && this._devices[hwid].onInterrupt) {
                        this.paused = this._devices[hwid].onInterrupt(function(){ this.paused = false; }.bind(this));
                    }
                    break;
            }
            if (this._breakpoints[this.mem.pc] === true) {
                this.stop();
            }
        }
    },
    ...
    reset: function() {
        ...
        // TODO: This won't work now. Need to property enumerate
    ...
    removeListener: function (listener) {
      var get = this._getListeners.indexOf(listener),
          set = this._setListeners.indexOf(listener),
          end = this._endListeners.indexOf(listener);

      if ( get !== -1 ) {
        this._getListeners.splice(get, 1);
      }
      if ( set !== -1 ) {
        this._setListeners.splice(set, 1);
      }
      if ( get !== -1 ) {
        this._endListeners.splice(end, 1);
      }
    },
    registerLabels: function (register) {
        var labels = this.addressesToLabels[register];
        var output = "";
        if (labels) { output += labels.join(" "); }
        var valueLabels = this.addressesToLabels[this.mem[register]];
        if (valueLabels) { output += " -> " + valueLabels.join(" "); }
        return output;
    },
    getDump: function() {
        ...
        output += 'I:  ' + formatWord(this.mem.i) + ' ' +
            this.registerLabels(this.mem.i) + '\n';
        output += 'J:  ' + formatWord(this.mem.j) + ' ' +
            this.registerLabels(this.mem.j) + '\n\n';
        output += 'PC: ' + formatWord(this.mem.pc) + ' ' +
            this.registerLabels(this.mem.pc) + '\n';
        ...
        for( i = 0; i < this.ramSize; i += 8) {
            ...
            if(populated) {
                ...
                for( j = 0; j < 8; j++) {
                    ...
                    if (this.showLabels) {
                        if (this.addressesToLabels.hasOwnProperty(i + j)) {
                          var a = this.addressesToLabels[i + j];
                          Array.prototype.push.apply(labels, a);
                        }
                    }
                }
                if (this.showLabels) {
